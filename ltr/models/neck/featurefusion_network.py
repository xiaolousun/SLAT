...
class CrossAttentionNetwork(nn.Module):
    ...
    def __init__(self, d_model=512, nhead=8, num_featurefusion_layers=4,
        ...
        featurefusion_layer = CrossAttentionLayer(d_model, nhead, dim_feedforward, dropout, activation)
        self.encoder = Encoder(featurefusion_layer, num_featurefusion_layers)
...
class CrossAttentionMutitemplateNetwork(nn.Module):

    def __init__(self, d_model=512, nhead=8, num_featurefusion_layers=4,
                 dim_feedforward=2048, dropout=0.1, activation="relu", nums_templates=2):
        super().__init__()

        featurefusion_layer = CrossAttentionLayer(d_model, nhead, dim_feedforward, dropout, activation)
        self.encoder = Encoder(featurefusion_layer, num_featurefusion_layers)
        self.merge_search_features = nn.ConvTranspose2d(in_channels=d_model*nums_templates, out_channels=d_model, kernel_size=1, stride=1)
        # self.merge_search_features = nn.Sequential(nn.Linear(d_model*nums_templates, d_model*nums_templates, bias=True),
        #                                            nn.ReLU(),
        #                                            nn.Linear(d_model*nums_templates, d_model, bias=True),
        #                                            nn.Dropout(0.1)) 

        # self.merge_search_features = nn.Linear(d_model*nums_templates, d_model)
        ...
        self.d_model = d_model
        self.nhead = nhead
        self.nums_templates = nums_templates
    ...
    def forward(self, src_temp, mask_temp, src_search, mask_search, pos_temp, pos_search):
        # src_temp = src_temp.flatten(2).permute(2, 0, 1)
        # pos_temp = pos_temp.flatten(2).permute(2, 0, 1)
        src_temp = src_temp.flatten(3).permute(1, 3, 0, 2)
        pos_temp = pos_temp.flatten(3).permute(1, 3, 0, 2)
        src_search = src_search.flatten(2).permute(2, 0, 1)
        pos_search = pos_search.flatten(2).permute(2, 0, 1)
        mask_temp = mask_temp.flatten(2)
        mask_search = mask_search.flatten(1)

        resutls = defaultdict(list)
        for i in range(src_temp.shape[0]):

            memory_temp, memory_search = self.encoder(src1=src_temp[i].squeeze(0), src2=src_search,
                                                    src1_key_padding_mask=mask_temp[:, i, ...].squeeze(1),
                                                    src2_key_padding_mask=mask_search,
                                                    pos_src1=pos_temp[i].squeeze(1),
                                                    pos_src2=pos_search)
            resutls['temp'].append(memory_temp)
            resutls['search'].append(memory_search)

        cat_search_feat = torch.cat([search_feat for search_feat in resutls['search']], dim=-1)
        L, B, C = cat_search_feat.shape
        search_feats = self.merge_search_features(cat_search_feat.permute(1, 2, 0).reshape(B, C, int(pow(L, 0.5)), -1)).flatten(-2).permute(2, 0, 1)
        # search_feats =  resutls['search'][-1]
        template_feats = torch.cat([temp_feat for temp_feat in resutls['temp']], dim=0)
...
def build_CrossAttnention_network(dim, dropout, nhead, dim_feedforward, num_featurefusion_layers, nums_templates):
    if nums_templates == 1:
        return CrossAttentionNetwork(
            d_model=dim,
            dropout=dropout,
            nhead=nhead,
    ...
    elif nums_templates > 1:
        return CrossAttentionMutitemplateNetwork(
            d_model=dim,
            dropout=dropout,
            nhead=nhead,
            dim_feedforward=dim_feedforward,
