...
class PositionEmbeddingSine2(nn.Module):
    ...
    def forward(self, features, features_mask):
        x = features
        mask = features_mask
        assert mask is not None
        not_mask = ~mask
...
def build_position_encoding2(hidden_dim, position_embedding_type):
    N_steps = hidden_dim // 2
    if position_embedding_type in ('v2', 'sine'):
        # TODO find a better way of exposing other arguments
        position_embedding = PositionEmbeddingSine2(N_steps, normalize=True)
