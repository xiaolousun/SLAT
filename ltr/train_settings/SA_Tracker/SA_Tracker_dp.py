...
def run(settings):
    ...
    settings.env.workspace_dir = '/data01/xlsun/code/TransT-M/results/SA_Tracker_3temps'
    settings.env.tensorboard_dir = settings.env.workspace_dir + '/tensorboard/' 
    settings.logdir = settings.env.workspace_dir + '/logs/'

    # train settings
    settings.nums_templates = 3
    settings.total_epoches = 500
    settings.LR_down_eopch = 400
    settings.samples_per_epoch = 300 * settings.batch_size
    ...
    #                                         num_search_frames=1, num_template_frames=settings.nums_templates,
    #                                         samples_per_epoch=settings.samples_per_epoch, max_gap=100, processing=data_processing_train,
    #                                         frame_sample_mode='transt')
    ...
    optimizer = torch.optim.AdamW(param_dicts, lr=1e-3,
                                  weight_decay=1e-4)
    lr_scheduler = torch.optim.lr_scheduler.StepLR(optimizer, settings.LR_down_eopch)

    # Create trainer
    trainer = TransTLTRTrainer(actor, [loader_train], optimizer, settings, lr_scheduler)

    # save config
    config_dir = settings.env.workspace_dir.split('results')[0] + 'ltr/train_settings/' + settings.module_name + '/' + settings.script_name + '.py'
    config_save_dir = settings.env.workspace_dir + '/' + settings.script_name + '.py'
